...
const viteConfig = fs.readFileSync(path.join(root, 'apps/telemetry-inspector/vite.config.ts'), 'utf8');
...
assert(source.includes('.button-skribbl-duels:hover:not(:disabled) { background:var(--SCD_ACCENT_HOVER); }'), 'homepage launcher does not use the shared accent hover color');
...
assert(source.includes("element('button', 'scd-button primary scd-result-return', 'Return')"), 'Danger-styled Return result action is missing');
assert(source.includes("element('button', 'scd-button primary scd-result-new', 'New Match')"), 'Ranked-styled New Match result action is missing');
assert(source.includes("element('button', 'scd-button primary scd-result-rematch', 'Rematch')"), 'Casual-styled Rematch result action is missing');
...
assert(source.includes("element('div', 'scd-chat-rematch-request')"), 'Opponent Rematch requests are not rendered in Match Chat');
