...
import {
  ...
  normalizeMatchChatCommandPrefix,
...
interface CompletionMessage {
  ...
  occurredAt: number;
  nameColorIndex?: number | undefined;
}
...
interface MatchChatMirrorMessage {
  messageId: string;
  author: string;
  message: string;
  avatar: HTMLElement;
...
const DUEL_PROFILE_STATUS_MAX_LENGTH = 80;

const ABOUT_TUTORIAL_PAGES = [
  {
...
function normalizeDuelProfileStatusText(value: unknown): string {
  return typeof value === 'string'
    ? Array.from(value.replace(/\s+/g, ' ').trim()).slice(0, DUEL_PROFILE_STATUS_MAX_LENGTH).join('')
    : '';
}
...
function element<K extends keyof HTMLElementTagNameMap>(
  ...
  if (tag === 'button' || tag === 'input' || tag === 'select' || tag === 'textarea') {
    ...
    for (const eventName of ISOLATED_POINTER_EVENTS) {
      // The modal scroll root must see wheel events so it can either scroll an
      // internal container or cancel scroll chaining into Skribbl's homepage.
...
function pinnedStatisticOrdinal(slot: 0 | 1): string {
...
class CompletionChatAdapter {
  ...
  private pendingMatchChat: MatchChatMirrorMessage[] = [];
  private mountGuard: number | null = null;

  public constructor(
    private readonly enabled: () => boolean,
    private readonly matchChatEnabled: () => boolean,
  ...
  public insertMatchChat(message: MatchChatMirrorMessage): void {
...
#game-chat .chat-content p.skribbl-duels-completion { padding:.2em;margin:0;opacity:1;animation:chat_msg .1s ease-in-out 1; }
...
#game-chat .chat-content p.skribbl-duels-completion.match-chat { display:flex;align-items:center;gap:.35em;color:var(--COLOR_PANEL_BUTTON) !important; }
...
html[data-scd-scroll-lock-runtime],body[data-scd-scroll-lock-runtime] { overflow:hidden !important;overscroll-behavior:none !important; }
#skribbl-duels-panel [data-role='body'],.scd-stage-shell,.scd-chat-log { overscroll-behavior:contain; }
[data-scd-runtime-id]::-webkit-scrollbar,[data-scd-runtime-id] ::-webkit-scrollbar { width:14px;height:14px;border-radius:7px;background-color:var(--COLOR_PANEL_LO) !important; }
[data-scd-runtime-id]::-webkit-scrollbar-track,[data-scd-runtime-id] ::-webkit-scrollbar-track { border-radius:7px;background-color:var(--COLOR_PANEL_LO) !important; }
[data-scd-runtime-id]::-webkit-scrollbar-thumb,[data-scd-runtime-id] ::-webkit-scrollbar-thumb { min-height:28px;border:0;border-radius:7px;background-color:var(--COLOR_PANEL_HI) !important;background-clip:border-box; }
[data-scd-runtime-id]::-webkit-scrollbar-thumb:hover,[data-scd-runtime-id] ::-webkit-scrollbar-thumb:hover,[data-scd-runtime-id]::-webkit-scrollbar-thumb:focus,[data-scd-runtime-id] ::-webkit-scrollbar-thumb:focus,[data-scd-runtime-id]::-webkit-scrollbar-thumb:active,[data-scd-runtime-id] ::-webkit-scrollbar-thumb:active { background-color:var(--COLOR_PANEL_HI) !important; }
[data-scd-runtime-id]::-webkit-scrollbar-button,[data-scd-runtime-id] ::-webkit-scrollbar-button { display:none;width:0;height:0; }
[data-scd-runtime-id],[data-scd-runtime-id] * { scrollbar-color:var(--COLOR_PANEL_HI) var(--COLOR_PANEL_LO);scrollbar-width:auto; }
...
.scd-queue-loader { grid-area:load;width:48px;aspect-ratio:1/1;justify-self:end;align-self:center;background-image:url('/img/load.gif');background-position:center;background-repeat:no-repeat;background-size:contain;animation:scd-queue-load-rotate .8s ease-in-out infinite; }
...
.scd-auth-avatar { width:42px;height:42px;border-radius:50%;object-fit:cover;background:transparent;flex:none; }
...
.scd-button.danger.scd-profile-status-reset,.scd-button.danger.scd-profile-status-reset:hover:not(:disabled),.scd-button.danger.scd-profile-status-reset:active:not(:disabled) { background:transparent; }
...
.scd-profile-status-input-shell { position:relative;min-width:0;display:flex; }
.scd-profile-status-input-shell input { padding-right:3em; }
...
.scd-profile-choice { min-height:74px;display:flex;align-items:center;gap:8px;background:var(--SCD_ACCENT);text-align:left; }
.scd-profile-choice:hover:not(:disabled) { background:var(--SCD_ACCENT_HOVER); }
.scd-profile-choice:active:not(:disabled) { background:var(--SCD_ACCENT_ACTIVE); }
.scd-profile-choice .scd-icon { width:38px;height:38px;flex:none; }
.scd-profile-choice.selected { outline:0;background:#53e237; }
.scd-profile-choice.selected:hover:not(:disabled) { background:#38c41c; }
.scd-profile-choice.selected:active:not(:disabled) { background:#30aa19; }
...
.scd-about-dots { width:100%;height:100%;display:flex;justify-content:center; }
.scd-about-dot { height:100%;width:auto;aspect-ratio:1;display:flex;justify-content:center;align-items:center;border:0;padding:0;background:transparent;opacity:.3;transition:opacity .1s ease-in-out;cursor:pointer; }
.scd-about-dot:hover,.scd-about-dot.active { background:transparent;opacity:1; }
.scd-about-dot-inner { width:44%;height:44%;background:#fff;border-radius:999px;box-shadow:0 0 3px rgba(0,0,0,.6); }
.scd-about-dot.active .scd-about-dot-inner { width:57%;height:57%; }
...
.scd-chat-log { min-height:0;max-height:330px;overflow:auto;min-width:0;transition:mask-image .12s;-webkit-transition:-webkit-mask-image .12s; }
...
@media (max-width:620px) {
  ...
  .scd-about-layout { grid-template-columns:minmax(0,1fr); }
  .scd-about-contact { grid-column:auto; }
}
@media (prefers-reduced-motion:reduce) {
  .scd-intro-logo,.scd-countdown-phase,.scd-field.drafted,.scd-final-slot-name,.scd-win-animation,.scd-win-player,.scd-duel-toast,.scd-queue-loader,.scd-about-page-visual { animation:none !important; }
}
`;
  ...
  private flush(): void {
    const completionEnabled = this.enabled();
    if ((!completionEnabled || (this.pending.length === 0 && this.pendingWins.length === 0))
        && this.pendingMatchChat.length === 0) return;
    const target = document.querySelector<HTMLElement>('#game-chat .chat-content');
    if (!target) return;

    while (completionEnabled && this.pending.length > 0) {
    ...
    while (completionEnabled && this.pendingWins.length > 0) {
    ...
    while (this.pendingMatchChat.length > 0) {
      const message = this.pendingMatchChat.shift();
      if (!message) break;
      const messageId = `match-chat:${message.messageId}`;
      if (target.querySelector(`[data-skribbl-duels-match-chat-id="${CSS.escape(messageId)}"]`)) continue;
      const paragraph = document.createElement('p');
      paragraph.className = 'skribbl-duels-completion match-chat';
      paragraph.dataset.scdRuntimeId = this.runtimeId;
...
export class DuelProductFoundation {
  ...
  private profileWordSort: NonNullable<LocalWordStatsQuery['sort']> = 'occurrence';
  ...
  private readonly playedCompletionSoundIds = new Set<string>();
  ...
  private inviteTypoNoticeShown = false;
  private typoDetected = false;
  ...
  public constructor(private readonly options: ProductFoundationOptions) {
    ...
    this.typoDetected = this.isTypoDetected();
    this.homepageMatchmakingAuthority = createHomepageMatchmakingAuthority(
      this.isHomepageDomVisible(),
      options.getLobbyAuthoritySnapshot()
    );
    if (this.lastTelemetryEvent) {
    ...
    this.chatAdapter = new CompletionChatAdapter(
      ...
      () => this.settings.matchChatMessages,
      options.runtimeId
    );
  ...
  public start(): ProductPublicApi {
    ...
    const api: ProductPublicApi = {
      ...
      gateway: {
        ...
        createInvite: format => {
          if (!this.isMatchmakingAvailable()) throw new Error(this.matchmakingUnavailableMessage());
          return this.gatewayClient.createInvite(format);
        },
        acceptInvite: token => {
          if (!this.isMatchmakingAvailable()) throw new Error(this.matchmakingUnavailableMessage());
          return this.gatewayClient.acceptInvite(token);
        },
        ...
        requestRematch: matchId => {
          if (!this.isMatchmakingAvailable()) throw new Error(this.matchmakingUnavailableMessage());
          return this.gatewayClient.requestRematch(matchId);
        },
  ...
  public destroy(reason = 'runtime-disposed'): void {
    ...
    document.querySelectorAll<HTMLElement>('[data-scd-match-chat-command-preview]').forEach(node => node.remove());
...
`;
  ...
  private createHomeButton(): HTMLButtonElement {
    ...
    button.type = 'button';
    button.append(
      this.createIconAsset('res/challenge-icons/skribbl-duels-logo.gif', 'SD', 'Skribbl Duels logo'),
  ...
  private createLauncher(): HTMLButtonElement {
    ...
    const icon = this.createIconAsset('res/challenge-icons/skribbl-duels-logo.gif', 'SD', 'Skribbl Duels');
  ...
  private createPanel(): HTMLDivElement {
    ...
    about.type = 'button';
    about.appendChild(this.createIconAsset('res/challenge-icons/about.gif', '?', 'About'));
  ...
  private createChallengeIcon(challengeId: string, className = 'scd-field-icon'): HTMLSpanElement {
    ...
    const icon = assetPath && EMBEDDED_ICON_ASSETS[assetPath]
  ...
  private openWithIntroduction(): void {
    ...
    const logo = this.createIconAsset(
      'res/challenge-icons/skribbl-duels-logo.gif',
      'SD',
      'Skribbl Duels'
    );
  ...
  private syncPageScrollLock(): void {
    const isVisible = (node: HTMLElement | null): boolean => Boolean(
      node?.isConnected && node.style.display !== 'none'
    );
    const locked = isVisible(this.panel)
      || isVisible(this.stage)
    ...
    for (const node of [document.documentElement, document.body]) {
      if (!node) continue;
      if (locked) {
        node.dataset.scdScrollLockRuntime = this.options.runtimeId;
      } else if (node.dataset.scdScrollLockRuntime === this.options.runtimeId) {
        delete node.dataset.scdScrollLockRuntime;
      }
    }
  }
  ...
  private releasePageScrollLock(): void {
    for (const node of [document.documentElement, document.body]) {
      if (node?.dataset.scdScrollLockRuntime === this.options.runtimeId) {
        delete node.dataset.scdScrollLockRuntime;
      }
    }
  }
  ...
  private renderVersusStage(
    ...
    ready.type = 'button';
    ready.append(
      this.createIconAsset('res/challenge-icons/checkmark.gif', '✓', 'Ready'),
    ...
    cancel.type = 'button';
    cancel.append(
      this.createIconAsset('res/challenge-icons/crossmark.gif', '×', 'Cancel'),
  ...
  private createVersusPlayer(
    ...
    status.append(
      this.createIconAsset(
        ready ? 'res/challenge-icons/checkmark.gif' : 'res/challenge-icons/crossmark.gif',
        ready ? '✓' : '×',
      ...
      if (flight.dataset.scdCountdownPhase !== phase) {
        ...
        if (phase !== 'waiting') {
          ...
          const assetPaths = phase === 'GO'
            ? [
              'res/challenge-icons/countdown_G.gif',
              'res/challenge-icons/countdown_O.gif',
              'res/challenge-icons/countdown_ExclamationMark.gif'
            ]
            : [`res/challenge-icons/countdown_${phase}.gif`];
  ...
  private openProfileStatusEditor(): void {
    this.createProfileDetail('Edit profile status', body => {
      ...
      const inputShell = element('span', 'scd-profile-status-input-shell');
      const characterCount = element(
        'span',
        'scd-chat-characters',
        String(Array.from(input.value).length)
      );
      const updateCharacterCount = (): void => {
        const codePoints = Array.from(input.value);
        if (codePoints.length > DUEL_PROFILE_STATUS_MAX_LENGTH) {
          input.value = codePoints.slice(0, DUEL_PROFILE_STATUS_MAX_LENGTH).join('');
        }
        const length = Array.from(input.value).length;
        characterCount.textContent = String(length);
        characterCount.classList.toggle('visible', length > 0);
      };
      input.addEventListener('input', updateCharacterCount);
      inputShell.append(input, characterCount);
      label.append(
        inputShell,
  ...
  private openAllProfileStats(): void {
    this.createProfileDetail('All local statistics', body => {
      ...
      if (this.profileWordLanguageId !== null
          && !languages.some(language => language.languageId === this.profileWordLanguageId)) {
        this.profileWordLanguageId = null;
      }

      const createCoverageCard = (
        ...
        card.append(
          ...
          element('div', 'scd-muted', `${seen.toLocaleString()} seen · ${guessed.toLocaleString()} guessed`),
          element('div', '', official === null
            ? 'Coverage unavailable'
            : `${seenCoverage ?? 0}% seen · ${guessedCoverage ?? 0}% guessed`)
        );
      ...
      if (languages.length > 0) {
        const totals = languages.reduce((result, language) => {
          ...
          return result;
        }, { seen: 0, guessed: 0, official: 0, coveredSeen: 0, coveredGuessed: 0 });
        const coveragePercent = (count: number): number | null => totals.official > 0
          ? Math.round(((count / totals.official) * 100 + Number.EPSILON) * 100) / 100
          : null;
        coverage.appendChild(createCoverageCard(
          null,
          'All languages',
          totals.seen,
          totals.guessed,
          totals.official > 0 ? totals.official : null,
          coveragePercent(totals.coveredSeen),
          coveragePercent(totals.coveredGuessed)
        ));
      }
      ...
      for (const language of languages) {
        coverage.appendChild(createCoverageCard(
          language.languageId,
          language.languageName ?? `Language ${language.languageId}`,
          language.uniqueWordsSeen,
          language.uniqueWordsGuessed,
          language.officialWordCount,
          language.seenCoveragePercent,
          language.guessedCoveragePercent
        ));
      }
      if (languages.length === 0) {
      ...
      const selectedLanguage = this.profileWordLanguageId === null
        ? null
        : languages.find(language => language.languageId === this.profileWordLanguageId) ?? null;
      body.appendChild(element(
        'strong',
        'scd-profile-section-title',
        `Most frequently seen words · ${selectedLanguage?.languageName ?? (selectedLanguage ? `Language ${selectedLanguage.languageId}` : 'All languages')}`
      ));
      ...
      const columns: ReadonlyArray<{
        label: string;
        sort: NonNullable<LocalWordStatsQuery['sort']>;
        defaultDirection: NonNullable<LocalWordStatsQuery['direction']>;
      }> = [
        { label: 'Word', sort: 'alphabetical', defaultDirection: 'ascending' },
        { label: 'Language', sort: 'language', defaultDirection: 'ascending' },
        { label: 'Seen', sort: 'occurrence', defaultDirection: 'descending' },
        { label: 'Guessed', sort: 'guessed', defaultDirection: 'descending' },
        { label: 'Avg WPM', sort: 'average-wpm', defaultDirection: 'descending' },
        { label: 'Avg guess', sort: 'average-guess-time', defaultDirection: 'ascending' }
      ];
      for (const column of columns) {
        const active = this.profileWordSort === column.sort;
        const cell = element('th');
        cell.setAttribute('aria-sort', active
          ? (this.profileWordSortDirection === 'ascending' ? 'ascending' : 'descending')
          : 'none');
        const sortButton = element('button', 'scd-profile-word-sort') as HTMLButtonElement;
        sortButton.type = 'button';
        sortButton.append(
          element('span', '', column.label),
          element(
            'span',
            'scd-profile-word-sort-indicator',
            active ? (this.profileWordSortDirection === 'ascending' ? '↑' : '↓') : ''
          )
        );
        sortButton.addEventListener('click', () => {
          if (active) {
            this.profileWordSortDirection = this.profileWordSortDirection === 'ascending'
              ? 'descending'
              : 'ascending';
          } else {
  ...
  private openDuelProfile(): void {
    ...
    const statusIcon = element('button', 'scd-button scd-profile-status-button-icon') as HTMLButtonElement;
  ...
  private createFinishedMatchSummary(): HTMLDivElement {
    ...
    const homepageAvailable = this.isMatchmakingAvailable();
    newMatch.disabled = !this.matchState.format || !homepageAvailable || this.gatewayState.status !== 'connected';
    if (!homepageAvailable) newMatch.title = this.matchmakingUnavailableMessage();
    ...
    rematch.disabled = !gatewayMatch
      ...
      || !homepageAvailable
      || this.gatewayState.status !== 'connected';
    if (!homepageAvailable) rematch.title = this.matchmakingUnavailableMessage();
    if (ownRematchReady) rematch.textContent = 'Rematch requested';
    rematch.addEventListener('click', () => {
      if (!gatewayMatch || !this.isMatchmakingAvailable()) return;
  ...
  private renderChatTab(): void {
      ...
      for (const message of this.duelChatMessages) {
        ...
        if (message.side === 'self' || message.side === 'opponent') {
          appendColoredDuelName(
            author,
            message.author,
            message.nameColorIndex ?? this.duelNameColorIndex(message.side)
          );
    ...
    if (rematchRequester) {
      ...
      if (!homepageAvailable) accept.title = this.matchmakingUnavailableMessage();
      accept.addEventListener('click', () => {
        if (!snapshot || !this.isMatchmakingAvailable()) return;
  ...
  private handleDuelChatKeydown(event: KeyboardEvent): void {
    ...
    if (typoCommandInput) {
      const original = document.querySelector<HTMLInputElement>(
        '#newChat, #game-chat input:not(#typo-command-input):not([type="hidden"])'
      );
      if (original) {
        original.value = '';
    ...
    if (!message) {
      this.showSimpleToast(
        'Match chat command',
        `Use ${this.settings.matchChatCommandPrefix} followed by a message.`
      );
      return;
    }
    if (!this.submitDuelChatMessage(message, false, false)) {
      this.showSimpleToast('Match chat unavailable', 'An active connected Duel match is required.');
    }
  }

  private submitDuelChatMessage(
    rawMessage: string,
    ...
    const gatewayChatActive = this.gatewayState.status === 'connected'
      ...
      && this.gatewayState.match?.state.phase !== 'cancelled';
    if (!message || !gatewayChatActive || !matchId) return false;

    this.restoreDuelChatFocus = restoreFocus;
  ...
  private renderSettingsTab(): void {
    ...
    if (this.authState.status === 'signed-in' && identity) {
      ...
      const colorButton = element('button', 'scd-button scd-profile-color-button') as HTMLButtonElement;
    ...
    const wpmDisplay = element('select') as HTMLSelectElement;
    ...
    wpmDisplay.addEventListener('change', () => this.settingsStore.update({
      wpmChatDisplay: wpmDisplay.value as ProductUiSettings['wpmChatDisplay']
    }));
    wpmDisplayLabel.append(element('span', '', 'Show WPM stat display'), wpmDisplay);

    const guessTimeDisplayLabel = element('label', 'scd-label');
    const guessTimeDisplay = element('select') as HTMLSelectElement;
    for (const [value, label] of [
      ['disabled', 'Disabled'],
    ...
    guessTimeDisplay.addEventListener('change', () => this.settingsStore.update({
      guessTimeChatDisplay: guessTimeDisplay.value as ProductUiSettings['guessTimeChatDisplay']
    }));
    ...
    const commandInput = element('input') as HTMLInputElement;
    ...
    const commitCommandPrefix = (): void => {
      const normalized = normalizeMatchChatCommandPrefix(commandInput.value);
      commandInput.value = normalized;
      if (normalized !== this.settings.matchChatCommandPrefix) {
        this.settingsStore.update({ matchChatCommandPrefix: normalized });
      }
      this.ensureTypoCommandPreview();
    };
    ...
    chat.append(
      element('strong', '', 'UI integration'),
      this.checkbox('Show challenge completions in skribbl chat', this.settings.completionMessages, checked =>
        this.settingsStore.update({ completionMessages: checked })),
      wpmDisplayLabel,
      guessTimeDisplayLabel,
      ...
      this.checkbox('Show Match Chat messages in skribbl chat', this.settings.matchChatMessages, checked =>
        this.settingsStore.update({ matchChatMessages: checked })),
      commandLabel,
      this.checkbox('Display win animation', this.settings.winAnimation, checked =>
    ...
    const volumeValue = element(
      'span',
      'scd-volume-value',
      this.settings.sfxVolume === 0 ? 'Muted' : `${this.settings.sfxVolume}%`
    );
    volumeTitle.append(volumeIcon, element('span', '', 'Volume'), volumeValue);
    const volumeInput = element('input') as HTMLInputElement;
  ...
  private renderAboutTab(): void {
    ...
    const tutorialHeading = element('div', 'scd-about-tutorial-heading');
    const tutorialLogo = this.createIconAsset(
      'res/challenge-icons/skribbl-duels-logo.gif',
      'SD',
      'Skribbl Duels logo'
    );
    tutorialLogo.classList.add('scd-about-tutorial-logo');
    tutorialHeading.append(tutorialLogo, element('strong', '', 'How Skribbl Duels works'));
    const pages = element('div', 'scd-about-pages');
    pages.setAttribute('aria-live', 'polite');
    const pageNodes = ABOUT_TUTORIAL_PAGES.map((page, index) => {
      const node = element('div', 'scd-about-page');
      node.dataset.scdTutorialPage = String(index);
      const visual = this.createIconAsset(page.assetPath, String(index + 1), page.title);
      visual.classList.add('scd-about-page-visual');
      node.append(
        visual,
        element('strong', 'scd-about-page-title', page.title),
        element('div', 'scd-muted scd-about-page-description', page.description)
      );
      pages.appendChild(node);
      return node;
    });
    const navigation = element('div', 'scd-about-navigation');
    const dots = element('div', 'scd-about-dots');
    dots.setAttribute('role', 'tablist');
    const dotNodes = ABOUT_TUTORIAL_PAGES.map((page, index) => {
      const dot = element('button', 'scd-about-dot') as HTMLButtonElement;
      dot.type = 'button';
      dot.setAttribute('role', 'tab');
      dot.setAttribute('aria-label', `Show tutorial page ${index + 1}: ${page.title}`);
      dot.appendChild(element('span', 'scd-about-dot-inner'));
      dots.appendChild(dot);
      return dot;
    });
    navigation.appendChild(dots);

    const selectPage = (index: number, restartTimer: boolean): void => {
      const pageIndex = (index + ABOUT_TUTORIAL_PAGES.length) % ABOUT_TUTORIAL_PAGES.length;
      this.aboutTutorialPageIndex = pageIndex;
      pageNodes.forEach((page, itemIndex) => {
        const active = itemIndex === pageIndex;
        page.classList.toggle('active', active);
        page.setAttribute('aria-hidden', String(!active));
      });
      dotNodes.forEach((dot, itemIndex) => {
        const active = itemIndex === pageIndex;
    ...
    let lastWheelAt = 0;
    tutorial.addEventListener('wheel', event => {
    ...
    startAutoAdvance();
    tutorial.append(tutorialHeading, pages, navigation);

    const contact = element('div', 'scd-card scd-about-contact');
    contact.append(
      this.createIconAsset('res/about-icons/contact.gif', '@', 'Contact'),
      element('div', 'scd-about-contact-copy')
    );
    const contactCopy = contact.querySelector<HTMLElement>('.scd-about-contact-copy');
    contactCopy?.append(
      element('strong', '', 'Contact'),
      element('span', 'scd-muted', 'Discord: analphabetism#0')
    );

    layout.append(copy, tutorial, contact);
  ...
  private isHomepageDomVisible(): boolean {
  ...
  private isHomepageVisible(): boolean {
    return this.isHomepageDomVisible()
      && isHomepageTelemetryEligible(this.homepageMatchmakingAuthority);
  }
  ...
  private beginMatchmaking(format: 'casual' | 'ranked'): string {
    ...
    if (!this.isHomepageVisible()) {
      this.matchmakingError = 'Matchmaking requires the visible Skribbl homepage and a confirmed lobby-exit Telemetry state.';
      this.renderPanel();
      throw new Error(this.matchmakingError);
    }
  ...
  private handleInviteAuthenticationState(): void {
    ...
    if (this.authState.status === 'initializing'
  ...
  private handleGatewayRealtimeState(state: GatewayConnectionSnapshot): void {
    ...
    for (const message of state.duelChatMessages) {
      ...
      const optimisticIndex = ownMessage
        ...
        : -1;
      const participant = state.match?.state.participants.find(item =>
        item.accountId === message.authorAccountId
      ) ?? null;
      ...
      if (message.occurredAt >= this.chatNotificationStartedAt - 5_000) {
        this.chatAdapter.insertMatchChat({
          messageId: message.messageId,
          author: message.authorDisplayName,
          message: message.message,
          avatar: this.createParticipantAvatar(
      ...
      if (!ownMessage
          && this.settings.matchChatPings
          && message.occurredAt >= this.chatNotificationStartedAt - 5_000) {
        this.soundEffects.play('matchChatPing');
      }
      if (!ownMessage
  ...
  private scheduleConclusionPresentation(state: MatchState, occurredAt: number): void {
    ...
    this.conclusionPresentationTimer = window.setTimeout(() => {
      ...
      if (snapshot.outcome === 'win' && snapshot.winner === 'self') {
        this.soundEffects.play('matchWin');
      }
      ...
      this.chatAdapter.insertWin({
        ...
        elapsedMs,
        nameColorIndex: snapshot.winner ? this.duelNameColorIndex(snapshot.winner) : undefined
      });
  ...
  private insertCompletion(message: CompletionMessage, mirrorToSkribbl = true): void {
    const coloredMessage: CompletionMessage = {
      ...message,
