...
async function main(): Promise<void> {
  ...
  assert(registered.length === 47, `Expected 47 registered definitions, got ${registered.length}.`);
  const activated = activateStarterSandbox(engine);
  assert(activated.length === 47, `Expected 47 activated instances, got ${activated.length}.`);
  ...
  console.log('All 47 starter challenges loaded; the legacy 46-Challenge fixture remains valid for its unchanged definitions.');
}
