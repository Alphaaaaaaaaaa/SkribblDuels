...
assert.equal(isTypoRuntimeDetected({ typoLoader: 'true' }), true);
assert.equal(isTypoRuntimeDetected({ typoLoaded: 'true' }), true);
assert.equal(isTypoRuntimeDetected({ typoLoader: 'false', typoLoaded: 'false' }), false);
assert.match(source, /Matchmaking is available only while the Typo loader is active/);
...
assert.equal(GATEWAY_CLIENT_VERSION, '0.63.0');
assert.match(userscriptSource, /const BUILD_VERSION = '0\.63\.0';/);
