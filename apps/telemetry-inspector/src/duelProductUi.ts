...
.scd-result-avatar.scd-avatar-skribbl .scd-skribbl-avatar,.scd-win-player.scd-avatar-skribbl .scd-skribbl-avatar { width:60%;height:60%; }
...
export class DuelProductFoundation {
  ...
  public destroy(reason = 'runtime-disposed'): void {
    ...
    if (window.skribblDuelsProduct?.version === '0.50.0') delete window.skribblDuelsProduct;
  }
...
`;
  ...
  private createFinishedMatchSummary(): HTMLDivElement {
    ...
    const returnButton = element('button', 'scd-button primary', 'Return') as HTMLButtonElement;
    returnButton.type = 'button';
    returnButton.addEventListener('click', () => {
      this.resetMatch();
      this.openPanel('duel');
    });
    const newMatch = element('button', 'scd-button primary', 'New Match') as HTMLButtonElement;
    ...
    const rematch = element('button', 'scd-button primary', 'Rematch') as HTMLButtonElement;
    rematch.type = 'button';
  ...
  private handleGatewayRealtimeState(state: GatewayConnectionSnapshot): void {
    ...
    for (const message of state.duelChatMessages) {
      ...
      if (!ownMessage
        ...
        this.showChatToast(message.authorDisplayName, message.message);
      }
    }
  ...
  private showChatToast(author: string, message: string): void {
  ...
  private scheduleConclusionPresentation(state: MatchState, occurredAt: number): void {
    ...
    this.conclusionPresentationTimer = window.setTimeout(() => {
      ...
      if (snapshot.outcome !== 'win' || !snapshot.winner) return;
