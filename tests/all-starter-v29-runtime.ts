...
async function main(): Promise<void> {
  ...
  const definitionsChangedAfterV29 = new Set([
  ...
  assert(registerStarterChallengeDefinitions(engine).length === 53, 'All 53 definitions should register.');
  assert(activateStarterSandbox(engine).length === 53, 'All 53 sandbox instances should activate.');
  ...
  for (const definition of starterChallengeDefinitions) {
    if (definitionsChangedAfterV29.has(definition.id)) continue;
